...
check(pkg.version === '1.3.2', 'v1.3.2 audit keeps application version 1.3.2');
...
check(
  ...
  'v1.3.2 user release verifier is registered'
);

check(
  typeof pkg.scripts?.check === 'string' && pkg.scripts.check.includes('npm run verify:pre132-user-release'),
  'v1.3.2 user release verification is permanent in npm run check'
);
...
if (failures > 0) {
  throw new Error(`Pre-1.3.2 user release verification failed: ${failures}/${checks} checks failed.`);
}

console.log(`Tubmedia v1.3.2 user release verification OK: ${checks} checks.`);
