 ...
 * Hiện dữ liệu THẬT của hàng đợi ghép (đã dùng chung ProcessManager với Ghép theo Timeline) và trỏ sang
...
export function StepMergeExportPage(): React.JSX.Element {
  ...
  return <div className="page-shell step-merge-export-page">
    ...
    <Card icon={Sparkles} title="Mở Ghép theo Timeline" subtitle="Chọn chất lượng xuất, theo dõi tiến độ và mở tệp kết quả">
      <div className="step-merge-cta">
        <p>Trang này chỉ tóm tắt; việc ghép và xuất thật diễn ra ở Ghép theo Timeline trong nhóm VIỆC CHÍNH.</p>
        <button type="button" className="btn btn-primary tm-btn-lg" onClick={() => setPage('download-merge')}>
          <FolderOpen size={16}/>
          Mở Ghép theo Timeline
        </button>
      </div>
    </Card>
  </div>;
