...
import '../logs-responsive-r35.css';
...
export function LogsPage(): React.JSX.Element {
  ...
  const clearSelected = async (): Promise<void> => {
    ...
    try {
      ...
      if (target) clearProjectLogs(target);
      else useAppStore.setState({ logs: [] });
      setLogs([]);
      setAttention({
        id: createUiEventId('logs-clear'),
        severity: 'info',
        title: target ? 'Đã xóa nhật ký khu vực đã chọn' : 'Đã xóa toàn bộ nhật ký',
        message: `Đã dọn ${result.removed} bản ghi. Tệp nhật ký đang mở cũng được làm mới an toàn.`,
        sticky: false
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="page-shell logs-page logs-responsive-page">
      ...
      <div className="page-action-toolbar logs-action-toolbar" aria-label="Công cụ nhật ký">
        <div className="page-action-group">
          <button className="btn" onClick={() => void window.desktop.logs.openFolder()}>
            <FolderOpen size={17} />
            Mở thư mục nhật ký
          </button>
          <button className="btn" onClick={() => void window.desktop.logs.exportDiagnostics()}>
            <Download size={17} />
            Gói chẩn đoán
          </button>
      ...
      <div className="logs-filter-grid mt-4 grid gap-3 xl:grid-cols-[minmax(260px,420px)_180px_minmax(220px,360px)]">
        <label>
          <span className="label">Nguồn nhật ký</span>
          <select
            className="select"
            value={projectId}
            onChange={(event: ChangeEvent<HTMLSelectElement>) => changeProject(event.target.value)}
          >
        ...
        <label>
          ...
          <input
            className="input"
            value={module}
            onChange={(event: ChangeEvent<HTMLInputElement>) => setModule(event.target.value)}
            placeholder="Ví dụ: tải xuống, hàng đợi, tiến trình"
          />
        </label>
      </div>
          ...
          <tbody>
            {filtered.map((entry) => (
              <tr key={entry.id}>
                ...
                <td className="logs-level-cell">
                  <StatusBadge status={entry.level} fixed />
                </td>
                <td className="logs-module-cell" title={moduleLabel(entry.module)}>
                  {moduleLabel(entry.module)}
                </td>
                <td className="logs-code-cell" title={entry.eventCode}>
                  {entry.eventCode}
                </td>
                <td className="logs-message-cell">
                  <div className="logs-message-compact">
                    <span>{entry.message}</span>
                    {shouldDiscloseMessage(entry.message) && (
                      <CompactDetail
                        label="Nội dung đầy đủ của nhật ký"
                        tone={
                          entry.level === 'error' ? 'danger' : entry.level === 'warn' ? 'warning' : 'info'
                        }
                      >
                        <p>{entry.message}</p>
                        <dl>
                          <div>
                            <dt>Thời gian</dt>
                            <dd>{new Date(entry.timestamp).toLocaleString('vi-VN')}</dd>
                          </div>
                          <div>
                            <dt>Thành phần</dt>
                            <dd>{moduleLabel(entry.module)}</dd>
                          </div>
                          <div>
                            <dt>Mã sự kiện</dt>
                ...
                <td className="logs-job-cell" title={entry.jobId ?? undefined}>
                  {entry.jobId ?? '—'}
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="py-10 text-center" style={{ color: 'var(--muted)' }}>
                  Chưa có nhật ký phù hợp bộ lọc.
                </td>
              </tr>
            )}
          </tbody>
